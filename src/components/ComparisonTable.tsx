    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-white bg-gray-900 rounded-3xl shadow-2xl max-w-7xl w-full overflow-hidden border border-gray-200 border-gray-700 max-h-[90vh] flex flex-col my-8"
        >
          {/* Header */}
          <div className="px-8 py-6 border-b border-gray-200 border-gray-700 bg-gradient-to-r from-beauty-soft/30 to-transparent flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              {comparisonProducts.length > 0 && (
                <button
                  ...
                  className="px-4 py-2 text-red-600 hover:bg-red-50 hover:bg-red-900/30 rounded-xl transition-colors font-medium"
                >
                  Tout effacer
                </button>
              )}
          ...
          <div className="flex-1 overflow-auto p-8">
            {comparisonProducts.length === 0 ? (
              <div className="text-center py-20">
                <TrendingUp className="w-20 h-20 text-gray-300 text-gray-600 mx-auto mb-6" />
                <h3 className="text-xl font-bold text-gray-700 text-gray-300 mb-2">
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b border-gray-200 border-gray-700">
                      <th className="text-left py-4 px-4 text-sm font-semibold text-gray-600 text-gray-400 uppercase tracking-wider w-32">
                        Attribut
                      </th>
                        ...
                        <th key={product.id} className="min-w-[250px] p-4">
                          <div className="flex flex-col items-center">
                            ...
                            <button
                              ...
                              className="mt-2 p-1 rounded-full hover:bg-red-50 hover:bg-red-900/30 text-red-500 transition-colors"
                            >
                              <X size={16} />
                            </button>
                          </div>
                  ...
                  <tbody>
                    {fields.map(field => (
                      <tr key={field.key} className="border-b border-gray-100 border-gray-800 hover:bg-gray-50 hover:bg-gray-800/50">
                        <td className="py-4 px-4 text-sm font-medium text-gray-700 text-gray-300">
                          {field.label}
                        </td>
                        {comparisonProducts.map(product => {
                          ...
                          if (field.type === 'image') {
                            ...
                            const colors: Record<string, string> = {
                              'à-apprendre': 'bg-amber-100 text-amber-800 bg-amber-900/30 text-amber-300',
                              'en-cours': 'bg-blue-100 text-blue-800 bg-blue-900/30 text-blue-300',
                              'maîtrisé': 'bg-emerald-100 text-emerald-800 bg-emerald-900/30 text-emerald-300'
                            };
                            displayValue = (
                            ...
                            displayValue = (
                              <p className="text-sm text-gray-600 text-gray-400 line-clamp-3">
                                {value}
                              </p>
                            );
                          ...
                          return (
                            <td
                              key={product.id}
                              className={`py-4 px-4 ${isDifferent ? 'bg-amber-50 bg-amber-900/20' : ''}`}
                            >
                              {displayValue}
                            </td>
                          );
          ...
          {/* Footer hint */}
          {comparisonProducts.length > 0 && (
            <div className="px-8 py-4 border-t border-gray-200 border-gray-700 bg-gray-50 bg-gray-800/50">
